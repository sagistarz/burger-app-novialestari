export default function Button({ children, variant = "primary", onClick, disabled }) {
    ...
    return (
      <button onClick={onClick} disabled={disabled} className={`py-2 px-6 w-full rounded-md font-bold text-sm block hover:opacity-90 active:scale-[0.98] ${variants[variant]} ${disabled && disabledStyle}`}>
        {children}
      </button>
    );
  }
